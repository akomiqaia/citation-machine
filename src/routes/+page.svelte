<script lang="ts">
	...
	let totalPages = 0;
	let currentPage = 0;
  let consoleMessage = 'This is a console message';
	async function handleQuery() {
		try {
			const resourceDirPath = await resourceDir();
      consoleMessage = resourceDirPath;
			const fetchSentenceQuery = await fetch(
				`http://localhost:8135/find-similar-sentences?q=${query}&data_dir=${resourceDirPath}`
			);
      const res = await fetchSentenceQuery.json();
      console.log(`🚀 ~ res:`, res);
			const result = (res as { similar_sentences: responseDetails[]  })
				.similar_sentences;
			response = result;
		} catch (e) {
			console.log(`🚀 ~ e:`, e);
	...
	async function handleUpload() {
		...
		const dataDirPath = await resourceDir();
    query = dataDirPath;
		files &&
			(await Promise.all(
	...
	const treeItems = [{ title: 'index.svelte', icon: 'file' }];
</script>
